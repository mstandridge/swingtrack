  ...
  if (pages.length === 0 || showImport) {
    return (
      <div className="max-w-lg mx-auto p-6">
        <h2 className="text-lg font-bold text-theater-dark mb-4">Import Script</h2>
        ...
        <button
          ...
          className="w-full border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-theater-purple transition mb-4"
        >
          <Upload size={32} className="mx-auto text-gray-400 mb-2" />
          <p className="text-sm text-gray-500">Upload a .txt file</p>
        </button>

        <div className="text-center text-xs text-gray-400 mb-4">— or paste your script below —</div>
        ...
        <textarea
          ...
          rows={12}
          className="w-full border border-gray-300 rounded-lg p-3 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-theater-purple"
        />
        <button
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-bold text-theater-dark">Script</h2>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowNotes(!showNotes)}
            className={`flex items-center gap-1 text-xs px-2 py-1 rounded transition ${
              showNotes ? 'bg-theater-purple text-white' : 'bg-gray-100 text-gray-500'
            }`}
          >
      ...
      <div className="bg-white rounded-lg border p-4 min-h-[50vh]">
      ...
      {showNotes && page && (
        <div className="mt-3 bg-white rounded-lg border overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 bg-gray-50 border-b">
            <span className="text-xs font-semibold text-gray-500 uppercase">Page Notes</span>
            ...
            <div className="relative">
              <select
                ...
                className="text-xs bg-white border rounded px-2 py-1 pr-6"
              >
                {activeTrack && (
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-xs font-medium text-gray-600">{trackName}</span>
      </div>
      {editable ? (
        <textarea
          value={note}
          ...
          rows={3}
          className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-theater-purple resize-y"
        />
      ) : (
        <p className="text-sm text-gray-700">{note || <span className="text-gray-300 italic">No notes</span>}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="border-t border-gray-100 pt-2">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-xs font-medium text-gray-400">{track.characterName}</span>
      </div>
      <p className="text-sm text-gray-500">{note || <span className="text-gray-300 italic">No notes</span>}</p>
    </div>
  );
}
